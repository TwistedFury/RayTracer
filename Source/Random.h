...
namespace random {
    ...
    // Generate a random glm::vec3 with each component in [min, max)
    inline glm::vec3 getReal(const glm::vec3& min, const glm::vec3& max) {
        return glm::vec3{
            getReal<float>(min.x, max.x),
            getReal<float>(min.y, max.y),
            getReal<float>(min.z, max.z)
        };
    }
    ...
    // Generate a random glm::vec3 with each component in [0, max)
    inline glm::vec3 getReal(const glm::vec3& max) {
        return getReal(glm::vec3{ 0.0f }, max);
    }
    ...
    inline glm::vec2 onUnitCircle() {
        ...
        return glm::vec2{ math::cosf(radians), math::sinf(radians) };
    }
